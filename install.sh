...
if [[ -f "$SOURCE_FILE" ]]; then
    if [[ -f "$TARGET_FILE.backup" ]]; then
        ...
        echo -e "${YELLOW}[i] Menghapus backup lama...${NC}"
    fi

    if [[ -f "$TARGET_FILE" ]]; then
        mv "$TARGET_FILE" "$TARGET_FILE.backup"
        echo -e "${YELLOW}[i] Backup: ${TARGET_FILE}.backup${NC}"
    fi
    ...
    echo "   Make sure the files lang_id.sh / lang_en.sh in this folder"
    exit 1
fi
...
echo ""
echo -e "${CYAN} Starting termux.sh...${NC}"

cd "$HOME" || {
    echo -e "${RED}❌ Fail Starting in Home.${NC}"
    exit 1
}
...
if [[ -f "$HOME/termux.sh" ]]; then
    ...
    echo -e "${RED}❌ termux.sh file Not found $HOME${NC}"
    exit 1
fi